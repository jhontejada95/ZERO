...
import { addressUrl, onchainProof, short, txUrl } from "./data/onchain-proof.js";
...
function EvidenceModal({ type, receipt, onClose }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal" role="dialog" aria-modal="true" onMouseDown={e => e.stopPropagation()}><button className="icon-button" onClick={onClose}><X size={20} /></button><p className="eyebrow">{approval ? "HUMAN AUTHORIZATION" : "AUDITABLE EVIDENCE"}</p><h2>{approval ? "Review before value moves." : "Every claim has a source."}</h2><p className="modal-intro">{approval ? "The authorization is bound to the exact program, beneficiary, amount, receipt hash and expiry. The EIP-712 safety canary has already settled on Base Sepolia." : "ZERO preserves provenance for every observation used to issue this receipt, including the paid x402 observation."}</p>{approval ? <div className="approval-summary"><span>BENEFICIARY</span><strong>{short(onchainProof.actors.beneficiary)}</strong><span>AMOUNT</span><strong>{onchainProof.receipt.amount}</strong><span>APPROVAL RAIL</span><strong>{onchainProof.humanApproval.standard} · Base Sepolia</strong><span>RECEIPT HASH</span><strong>{short(onchainProof.receipt.receiptHash)}</strong><span>APPROVAL DIGEST</span><strong>{short(onchainProof.humanApproval.digest)}</strong><span>CANARY TX</span><strong>{short(onchainProof.humanApproval.canarySettlement)}</strong></div> : <div className="evidence-list">{receipt.evidence.map(item => <div className="evidence-row" key={item.source}><Database size={22} /><div><strong>{item.name}</strong><span>{item.source} · {item.detail} · {item.hash}</span></div><CheckCircle size={19} weight="fill" /></div>)}<div className="evidence-row"><Database size={22} /><div><strong>x402 paid climate-risk observation</strong><span>{onchainProof.x402.amount} · tx {short(onchainProof.x402.payment)}</span></div><CheckCircle size={19} weight="fill" /></div></div>}<a className="modal-primary" href={approval ? txUrl(onchainProof.humanApproval.canarySettlement) : txUrl(onchainProof.x402.payment)} target="_blank" rel="noreferrer">{approval ? "Open authorization tx" : "Open x402 payment"}<ArrowRight size={18} /></a></section></div>;
}
...
function AgentModal({ onClose }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal agent-modal" role="dialog" aria-modal="true" onMouseDown={e => e.stopPropagation()}><button className="icon-button" onClick={onClose}><X size={20} /></button><p className="eyebrow">CONFIRMED PREVENTION RUN</p><h2>Every agent earns the right to settle.</h2><p className="modal-intro">Wallet creation, paid x402 evidence, verification, EAS attestation, human authorization and settlement remain one auditable Base Sepolia chain.</p>{status === "idle" && <div className="agent-launch"><div><Robot size={30} /><span>Bounded agent handoffs</span></div><div><Wallet size={30} /><span>Real testnet wallets</span></div><div><FlowArrow size={30} /><span>Confirmed settlement</span></div></div>}{status === "running" && <div className="agent-running"><i /><span>Loading confirmed on-chain proof…</span></div>}{status === "failed" && <p className="agent-error">The proof could not load. No funds or state were changed.</p>}{run && <ol className="agent-timeline">{run.timeline.map(event => <li key={event.index}><span>{String(event.index).padStart(2, "0")}</span><div><strong>{event.agent}</strong><small>{event.action}</small><p>{event.detail}</p></div><CheckCircle size={17} weight="fill" /></li>)}</ol>}<button className="modal-primary" disabled={status === "running"} onClick={status === "complete" ? onClose : launch}>{status === "complete" ? "Return to program" : status === "running" ? "Loading proof" : "Load on-chain proof"}<ArrowRight size={18} /></button></section></div>;
}
...
function ChainLink({ label, value, type = "tx" }) {
  const href = type === "address" ? addressUrl(value) : txUrl(value);
  return <a className="chain-link" href={href} target="_blank" rel="noreferrer"><span>{label}</span><code>{short(value)}</code><ArrowRight size={14} /></a>;
}

function OnchainSection({ section, roleKey, receipt, onBack, openModal }) {
  const role = roles[roleKey];
  const workspace = roleWorkspaces[roleKey];
  const cards = {
    programs: { title: roleKey === "operator" ? "Execution program room" : "Program capital view", intro: roleKey === "beneficiary" ? "Community-facing status only: what was protected, what was released and the public proof." : "La Bocana is the active on-chain prevention program. Each role sees only the controls it is allowed to use." },
    approvals: { title: "Human approval rail", intro: "The release is bound to an EIP-712 payload, an EAS attestation, an exact beneficiary, an exact amount and replay-protected escrow rules." },
    settlements: { title: "Settlement ledger", intro: "Every value movement shown here is a Base Sepolia transaction, not a visual placeholder." },
    evidence: { title: "Evidence registry", intro: "The evidence package includes a paid x402 observation, field records, counterfactual checks and a sealed receipt hash." },
    audit: { title: "Public audit trail", intro: "Auditors reconstruct the receipt from contracts, roles, attestation, x402 payment, approval digest and settlement transactions." },
  }[section];
  return <main className="onchain-workspace">
    <header><button onClick={onBack}><ArrowLeft size={17} /> Role home</button><p>ZERO · {section.toUpperCase()} · {role.label.toUpperCase()}</p><h1>{cards.title}</h1><blockquote>{cards.intro}</blockquote></header>
    <section className="onchain-grid">
      <article className="onchain-card primary"><span>SELECTED ROLE</span><h2>{workspace.surface}</h2><p>{workspace.thesis}</p><div>{workspace.can.map(item => <small key={item}><CheckCircle size={14} weight="fill" />{item}</small>)}</div></article>
      <article className="onchain-card"><span>RECEIPT</span><h2>{receipt.receiptId}</h2><p>{onchainProof.receipt.amount} · {onchainProof.receipt.status} · {onchainProof.network}</p><code>{onchainProof.receipt.receiptHash}</code></article>
      <article className="onchain-card"><span>CONTRACT BOUNDARY</span><h2>Escrow + EAS</h2><ChainLink label="Settlement escrow" value={onchainProof.contracts.SettlementEscrow} type="address" /><ChainLink label="Approval escrow" value={onchainProof.contracts.ApprovalEscrow} type="address" /><ChainLink label="EAS" value={onchainProof.contracts.EAS} type="address" /></article>
      {section === "approvals" && <article className="onchain-card wide amber-card"><span>IMPRESCINDIBLE · HUMAN AUTHORIZATION</span><h2>{onchainProof.humanApproval.standard}</h2><p>Approver {short(onchainProof.humanApproval.approver)} signed a replay-protected payload. The safety canary settled on Base Sepolia and proves the authorization rail works without moving the historical receipt twice.</p><ChainLink label="Canary settlement" value={onchainProof.humanApproval.canarySettlement} /><button onClick={() => openModal("approval")}>Inspect approval packet <ArrowRight size={16} /></button></article>}
      {section === "evidence" && <article className="onchain-card wide"><span>X402 PAID EVIDENCE</span><h2>{onchainProof.x402.amount}</h2><p>The procurement wallet paid the evidence provider in Base Sepolia USDC before the climate-risk resource returned the observation.</p><ChainLink label="x402 payment" value={onchainProof.x402.payment} /></article>}
      {section === "audit" && <article className="onchain-card wide"><span>RECONSTRUCTION ORDER</span><ol className="audit-steps">{["Program + escrow", "x402 evidence purchase", "Receipt hash", "EAS attestation", "Human EIP-712 approval", "Escrow settlement"].map((step, index) => <li key={step}><i>{index + 1}</i>{step}</li>)}</ol></article>}
      <article className="onchain-card full"><span>BASE SEPOLIA TRANSACTIONS</span><div className="tx-table">{onchainProof.transactions.map(([label, hash]) => <ChainLink key={hash} label={label} value={hash} />)}</div></article>
    </section>
  </main>;
}
